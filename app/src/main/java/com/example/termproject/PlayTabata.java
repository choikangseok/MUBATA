...
public class PlayTabata extends AppCompatActivity implements TextToSpeech.OnInitListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        alphaS = ((Button)findViewById(R.id.btn_start)).getBackground();
        alphaC = ((Button)findViewById(R.id.btn_cancel)).getBackground();
        alphaP = ((Button)findViewById(R.id.btn_pause)).getBackground();
        alphaR = ((Button)findViewById(R.id.btn_resume)).getBackground();



        spinmus = (Spinner)findViewById(R.id.spin1);

        nowsong = (TextView)findViewById(R.id.current_course);
        nextsong = (TextView)findViewById(R.id.next_course);

        prog= (ProgressBar)findViewById(R.id.progressBar1);
        prog2= (ProgressBar)findViewById(R.id.progressBar2);
        prog.setMax(100);
        prog.setScaleY(20f);
        prog.setProgressTintList(ColorStateList.valueOf(Color.rgb(255,150,0)));

        prog2.setMax(100);
        prog2.setScaleY(20f);
        prog2.setProgressTintList(ColorStateList.valueOf(Color.rgb(0,150,255)));
        ...
        btnStart.setOnClickListener(new View.OnClickListener(){
            ...
            public void onClick(View v){
                ...
                nowsong.setText(result[1] + " " + result[4]);
                nextsong.setText(result[5].substring(1, result[5].length()) + " " + result[8]);
                ...
                timer = new CountDownTimer(millisInFuture,countDownInterval){
                    public void onTick(long millisUntilFinished){
                        ...
                        if((first-millisUntilFinished) >= PreE){
                            N=N+4;
                            final String str = result[N-2];
                            nowsong.setText(str.substring(1, str.length()) + result[N] + " " + result[N+1]);
                            if(N+4 <= result.length-3) {
                                nextsong.setText(result[N+2].substring(1, str.length()) + " " + result[N+5]);
                            } else {
                                nextsong.setText("");
                            }
                        ...
                        if(isPaused || isCanceled)
                        {
                            ...
                            if( ((int)timepartRemaining /1000)==4 )
                                myTTS.speak("포올", TextToSpeech.QUEUE_FLUSH, null);
                            if( ((int)timepartRemaining /1000)==3 )
                                myTTS.speak("쓰리", TextToSpeech.QUEUE_FLUSH, null);
                            if( ((int)timepartRemaining /1000)==2 )
                                myTTS.speak("투우", TextToSpeech.QUEUE_FLUSH, null);
                            if( ((int)timepartRemaining /1000)==1 )
                                myTTS.speak("워언", TextToSpeech.QUEUE_FLUSH, null);
